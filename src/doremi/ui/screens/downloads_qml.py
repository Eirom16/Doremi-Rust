...
class DownloadsScreenQml(QWidget):
    ...
    def __init__(self, extractor, on_play_local, on_play_local_playlist=None, on_navigate=None):
        ...
        self._quick.setSource(QUrl.fromLocalFile(str(QML_DIR / "DownloadsScreen.qml")))

        self._load_ok = self._quick.status() == QQuickWidget.Status.Ready
        if not self._load_ok:
            for err in self._quick.errors():
                logger.error(f"QML DownloadsScreen: {err.toString()}")
